...
class LinkageParser
  ...
  def load_polygon(lines, start_index)
    ...
    if is_polygon_id_line?(line)
      ...
      while is_polygon_id_continue?(line = lines[start_index += 1]) do #check the next time is still lot line
        polygon_ids_lines += line.strip
      end
      polygon_start = true
    end
  ...
  def add_linkage_to_polygons(linkage_lines)
    polygon_linkages = @polygons[-1][:linkage_indexes] = []
    # puts "@parcels: #{@parcels[-1]}"
    linkage_lines.each do |line|
